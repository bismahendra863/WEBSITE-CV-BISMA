...
function initMobileMenu() {
  ...
  qsa(".nav__link", menu).forEach(function (a) {
    a.addEventListener("click", function () {
      ...
      if (href.charAt(0) === "#") setActiveNav(href.slice(1));
      setOpen(false);
    });
  });
}
...
function initScrollSpy() {
  ...
  function getCurrentSectionId() {
    ...
    if (document.elementFromPoint && document.documentElement) {
      var x = Math.min(24, Math.max(8, (document.documentElement.clientWidth || 0) * 0.15));
      var el = document.elementFromPoint(x, probe);
      var hit = el && el.closest ? el.closest("section[id]") : null;
...
function initPhotoModal() {
  var modal = qs("#photoModal");
  if (!modal) return;

  var img = qs("#photoImg", modal);
  var caption = qs("#photoCaption", modal);
  var title = qs("#photoTitle", modal);
  var closeBtn = qs(".modal__close", modal);
  if (!img || !caption || !title || !closeBtn) return;

  var lastFocus = null;

  function setOpen(open) {
    modal.classList.toggle("is-open", open);
    modal.classList.toggle("is-loading", false);
  ...
  function openWith(src, cap) {
    var s = String(src || "").trim();
    if (!s) return;
    lastFocus = document.activeElement;

    caption.textContent = cap ? String(cap) : "";
    if (cap) title.textContent = String(cap);
    else title.textContent = "Foto Kegiatan";
  ...
  qsa("[data-photo]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      openWith(btn.getAttribute("data-photo"), btn.getAttribute("data-caption"));
    });
  });
